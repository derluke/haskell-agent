...
import Agent.MCP
import Agent.OpenAI (OpenAIClient, newDataRobotClient, newOpenAIClient, sendOpenAIRequest)
import Agent.Tools
import Agent.Types
import Data.Aeson
...
data AgentEvent
  ...
  | -- | Tool name and result
    EventToolResult Text Text
  | -- | Model thinking/text before tools
    EventThinking Text
  | -- | Final text response
    EventModelResponse Text
  deriving (Show, Eq)
...
agent ::
  ...
  Text ->
  -- | Output parser
  (Value -> Either Text output) ->
  Agent () output
agent model systemPrompt parser =
  Agent
    { agentConfig =
        AgentConfig
          { acModel = model,
...
agentWithDeps ::
  ...
  (Value -> Either Text output) ->
  Agent deps output
agentWithDeps model systemPrompt parser =
  Agent
    { agentConfig =
        AgentConfig
          { acModel = model,
        ...
        Right (ToolUseResponse toolCalls usage) -> do
          ...
          case filter (\tc -> tcName tc == resultToolName) toolCalls of
            ...
            [] -> do
              ...
              mapM_
                (\(tc, tr) -> callback $ EventToolResult (tcName tc) (trContent tr))
